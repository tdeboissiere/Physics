...
int build_Gamma_tree(TString bolo_name)
{
    ...
    */
    ...
    TFile * file_Gamma = new TFile(gen_path+  TString("ROOT_files/Axion/Model/") + bolo_name +  TString("_model_perkeV.root"), "read");
    TF1 * func_FidGamma = (TF1*)file_Gamma->Get(bolo_name + TString("_FidGamma_deconv"));
    ...
    TString Gamma_out_path = gen_path + TString("/Scripts/Event_generation/ROOT_files/");
...
int main()
{
   ...
   {
        cout << "Simulating FidGamma for bolo " << *bolo_it << endl;
        build_Gamma_tree(*bolo_it);
    }
