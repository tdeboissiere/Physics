...
int build_signal_tree(TString bolo_name, TString axion_mass)
{
    ...
    */
    ...
    TFile * file_CBRD = new TFile(gen_path+  TString("ROOT_files/Axion/CBRD/CBRD_cross_section.root"), "read");
    TF1 * func_CBRD = (TF1*)file_CBRD->Get("CBRD_cross_mass_" + axion_mass);
    ...
    TFile * file_fwhm = new TFile(gen_path + TString("Scripts/Event_generation/ROOT_files/") + bolo_name +  TString("_fwhm_time_hist.root"), "read"); 
    ...
    TString Signal_out_path = gen_path + TString("/Scripts/Event_generation/ROOT_files/");
    TFile *ftree = new TFile(Signal_out_path + bolo_name+"_signal_tree.root", "update");
    ...
    TTree *t_new = new TTree("t_new" + axion_mass, "t_new" + axion_mass);

    float EIA, EIC, EIB, EID, FWIA, FWIC, EC, EFID, FWC, FWFID, Ebest, FWbest;
    ...
    t_new->Branch("Ebest", &Ebest, "Ebest/F");
    t_new->Branch("FWbest", &FWbest, "FWbest/F");


    for (int k=0; k<1000000; k++) 
    {

        float evt_heat = func_CBRD->GetRandom();
        ...
        EFID = 0.5*EIB+0.5*EID;
        float accept = ftrigger->Eval(EC);

        double coeff_EC = compute_coeff(FWC, FWFID);
        FWbest = compute_resolution(FWC,FWFID);
        Ebest = coeff_EC*EC + (1-coeff_EC)*EFID;
...
int main()
{
   ...
   vector<TString> mass_vec;
   mass_vec.push_back(TString("0"));
   mass_vec.push_back(TString("0.01"));
   mass_vec.push_back(TString("0.05"));
   mass_vec.push_back(TString("0.1"));
   mass_vec.push_back(TString("0.15"));
   mass_vec.push_back(TString("0.2"));
   ...
   {
        std::remove(TString("/home/irfulx204/mnt/tmain/Desktop/Run308_Axion/Scripts/Event_generation/ROOT_files/")  + *bolo_it+ TString("_signal_tree.root"));
        cout << "Simulating Signal for bolo " << *bolo_it << endl;
       for(std::vector<TString>::iterator mass_it = mass_vec.begin(); mass_it != mass_vec.end(); mass_it++) 
        {
            build_signal_tree(*bolo_it, *mass_it);
        }
    }
