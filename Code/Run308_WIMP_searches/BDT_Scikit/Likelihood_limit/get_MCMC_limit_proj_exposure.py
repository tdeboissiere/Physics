...
def get_projected_MCMC_limit(d_test, d_eval, d_event_dir, WIMP_mass, bolo_name, analysis_type, MVA_tag, exposure, bin_X, min_X, max_X, **kwargs):
    ...
    for niter in range(5):
        ...
        def lnprior(theta):
            ...
            if 0.9 < bckg_norm < 1.1  and 1E-2< WIMP_norm < 2000:
                return 0.0
            return -np.inf
        ...
        result = op.minimize(nll, [1, 1], bounds = ((1E-2,100), (1E-2, 2000)), args=(arr_data, arr_log_data, arr_bckg, arr_WIMP))
        m_ml= result["x"]
        print m_ml
...
with open("./Text_files/" + bolo_name + "_" + analysis_type + "_NWIMP_at_limit_MCMC_proj_" + str(int(exposure)) + "_jour" + ".txt", "w") as f:
    for WIMP_mass in list_mass:
