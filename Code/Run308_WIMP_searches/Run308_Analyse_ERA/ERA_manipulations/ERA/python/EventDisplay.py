...
class EventDisplay:
    def __init__(self,tkroot,anadir,bolo):
        ...
        self.TkVarAmplSeuil.set(str(0))
        self.TkVarUseAmplSeuil=Tk.IntVar()
        self.TkVarUseAmplSeuil.set(0)

        self.EvtListNumber=-1
        self.TkVarScanEvtList=Tk.IntVar()
        self.TkVarScanEvtList.set(0)
        ...
        # (un frame gauche et un frame droit; le frame droit divise en deux)
        # Frame gauche : le choix des voies et des runs et les operations sur le pulse
        ################################
        self.framegauche=Tk.Frame(tkroot)
        self.framegauche.pack(side=Tk.LEFT,padx=10,fill=Tk.BOTH)
        self.label_voies=Tk.Label(self.framegauche,text="Select channels:")
        self.label_voies.pack(side=Tk.TOP,pady=5)
        ...
        self.frameinferieur.pack(side=Tk.TOP,fill=Tk.BOTH)
        ...
        self.label_run=Tk.Label(self.frameinferieur,textvariable=self.TkVarRun)
        self.label_run2=Tk.Label(self.frameinferieur,text=":")
        self.label_nbevts=Tk.Label(self.frameinferieur,textvariable=self.TkVarNbEvts)
        self.label_nbevts2=Tk.Label(self.frameinferieur,text="events    ")
        self.label_nbevts2.pack(side=Tk.RIGHT)
        self.label_nbevts.pack(side=Tk.RIGHT)
        self.label_run2.pack(side=Tk.RIGHT)
        self.label_run.pack(side=Tk.RIGHT)

        # Un 2e frame en dessous
        ################################
        self.frameinferieur2=Tk.Frame(self.framedroit)
        self.frameinferieur2.pack(side=Tk.BOTTOM,pady=10,fill=Tk.BOTH)
        self.checkbutton_evtlist=Tk.Checkbutton(master=self.frameinferieur2,text="Scan event list",variable=self.TkVarScanEvtList)
        self.checkbutton_evtlist.pack(side=Tk.LEFT,anchor=Tk.W)
        self.checkbutton_amplseuil=Tk.Checkbutton(master=self.frameinferieur2,text="Use amplitude threshold:",variable=self.TkVarUseAmplSeuil)
        self.checkbutton_amplseuil.pack(side=Tk.LEFT,anchor=Tk.W)
        self.Entry_amplseuil=Tk.Entry(self.frameinferieur2,width=7,textvariable=self.TkVarAmplSeuil)
        self.Entry_amplseuil.pack(side=Tk.LEFT)
        self.button_quit = Tk.Button(master=self.frameinferieur2,text='Quit',command=sys.exit)
        self.button_quit.pack(side=Tk.RIGHT,padx=10)
    ...
    def next_evt(self):
        if self.TkVarScanEvtList.get()!=0 and self.TkVarUseAmplSeuil.get()!=0 :
            print "Unckeck one of the options: evt list OR ampl threshold !"
            return
        thenum=self.EvtNum+1
        if self.TkVarScanEvtList.get()!=0 :
            thenum=self.scanevtlist("fwd")
        if self.TkVarUseAmplSeuil.get()!=0 :
            thenum=self.scanevt_seuil("fwd")
        self.change_numevt(thenum)
    ...
    def prev_evt(self):
        ...
        thenum=self.EvtNum-1
        if self.TkVarScanEvtList.get()!=0 :
            thenum=self.scanevtlist("bkwd")
        if self.TkVarUseAmplSeuil.get()!=0 :
            thenum=self.scanevt_seuil("bkwd")
        self.change_numevt(thenum)
    ...
    def scanevtlist(self,direction):
        evtlist=RunParams.ReadListeEvts(self.BoloDir)
        if evtlist==None :
            print "No event list file"
            return
        # Mise a jour du numero courant dans la liste d'evts
        # NB variable EvtListNumber initialisee a -1
        if direction=="fwd" :
            self.EvtListNumber+=1
            if self.EvtListNumber>=len(evtlist) :
                print "Starting back at beginning of evt list"
                self.EvtListNumber=0
        elif direction=="bkwd" :
            self.EvtListNumber-=1
            if self.EvtListNumber<=-1 :
                print "Starting back at end of evt list"
                self.EvtListNumber=len(evtlist)-1
        else :
            print "ScanEvtList: Bad direction...!"
            return
        # Mise a jour run et numero d'evt
        ...
        self.TkVarRun.set(self.Run)
        self.updatetree()
        return thenum-1 # "-1" car le numero evt est en notation samba
    
    def scanevt_seuil(self,direction):
        ntpfile=self.ampldir+"basicntp_"+self.Run+"_"+self.bolo+".root"
        amplseuil=int(self.TkVarAmplSeuil.get())
        thenum=self.EvtNum
        if amplseuil==0 or not os.path.exists(ntpfile):
            print "Seuil not ok or no basicntp.."
        else :
            f=TFile(ntpfile)
            t=f.Get("basicntp_"+self.bolo)
            chtype=([type for type,name in self.channel_dict.items() if name==self.Channel])[0]
            while (thenum<self.NbEvts and thenum>=0) :
                if direction=="fwd" : thenum+=1
                elif direction=="bkwd" : thenum-=1
                else : print "Scanevt_seuil: Wrong direction!!!!"
                t.GetEntry(thenum)
                amplitude=eval("t.SimpleAmpl"+chtype)
                if amplseuil>0 and amplitude>amplseuil : break
                if amplseuil<0 and amplitude<amplseuil : break
            f.Close()
        return thenum

    def change_numevt(self,thenum) :
        if thenum>=self.NbEvts or thenum<0 :
            print "Wrong evt num!",thenum
        else :
            self.EvtNum=thenum
    ...
    def updatetree(self):
        ...
        if len(list_pulsefiles)==0 : print "Warning, no pulse files for this run"
