...
int get_trigged_trace_tree(TString bolo_name, string run_id)
{
  ...
  TString outdir =gen_dir + bolo_name + TString("/") + bolo_name +  TString("_") + Run_ID + TString("_trigged_event_tree.root");
  TFile *outf = new TFile(outdir, "recreate");
  TTree* out = new TTree("out", "out");

  out->Branch("Event","EdwEvent",&Evt); // default buffer size 32000  
