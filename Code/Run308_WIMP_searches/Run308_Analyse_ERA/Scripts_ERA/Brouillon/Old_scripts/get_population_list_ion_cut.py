...
def get_population_list(bolo_name, data_dir, tree_name):
    ...
    tree.Draw(">>l_all",standard_cuts +  "&&" + d_estimator["HEAT"] + "<40 &&" + d_estimator["HEAT"] + ">0 && abs(EC1-EC2)<2&& (EIB>1 || EID>1)")
    pop_len = l_all.GetN()
    pop_file_name = bolo_name + "_all_ion_cut.txt"
    ...
    #Get Fiducial gammas
    tree.Draw(">>l_GammaFid",standard_cuts+  "&&" + d_estimator["HEAT"] + "<40 &&" + d_estimator["HEAT"] + ">0 && EIA<" + d_FWHM["FWIA"] +" && EIB>" + d_FWHM["FWIB"] +"&& EIC<" + d_FWHM["FWIC"] +"&& EID>" + d_FWHM["FWID"] + "&& abs(EC1-EC2)<2 &&" +  d_estimator["Q_FID"]  + ">0.7&& (EIB>1 || EID>1)")
    pop_len = l_GammaFid.GetN()
    pop_file_name = bolo_name + "_GammaFid_ion_cut.txt"
    pop_file = script_utils.open_text_file(pop_path_name, pop_file_name, 'w')
    for k in range(pop_len):
    ...
    tree.Draw(">>l_GammaS1",standard_cuts +  "&&" + d_estimator["HEAT"] + "<40 &&" + d_estimator["HEAT"] + ">0  && EIA>" + d_FWHM["FWIA"] +" && EIB>" + d_FWHM["FWIB"] +"&& EIC<" + d_FWHM["FWIC"] +"&& EID<" + d_FWHM["FWID"] + "&& abs(EC1-EC2)<2 &&" + d_estimator["Q_S1"] + ">0.75&& (EIB>1 || EID>1)")
    # print standard_cuts +  "&&" + d_estimator["HEAT"] + ">0 && EIA>1 && EIB>1 && EIC<1 && EID<1 && abs(EC1-EC2)<2 && " + d_estimator["Q_S1"] + ">0.75"
    pop_len = l_GammaS1.GetN()
    pop_file_name = bolo_name + "_GammaS1_ion_cut.txt"
    pop_file = script_utils.open_text_file(pop_path_name, pop_file_name, 'w')
    for k in range(pop_len):
    ...
    tree.Draw(">>l_GammaS2",standard_cuts +  "&&" + d_estimator["HEAT"] + "<40 &&" + d_estimator["HEAT"] + ">0 && EIA<" + d_FWHM["FWIA"] +" && EIB<" + d_FWHM["FWIB"] +"&& EIC>" + d_FWHM["FWIC"] +"&& EID>" + d_FWHM["FWID"] + "&& abs(EC1-EC2)<2 &&" + d_estimator["Q_S2"] + ">0.75&& (EIB>1 || EID>1)")
    pop_len = l_GammaS2.GetN()
    pop_file_name = bolo_name + "_GammaS2_ion_cut.txt"
    pop_file = script_utils.open_text_file(pop_path_name, pop_file_name, 'w')
    for k in range(pop_len):
    ...
    tree.Draw(">>l_BetaS1",standard_cuts +  "&&" + d_estimator["HEAT"] + "<40 &&" + d_estimator["HEAT"] + ">0  && EIA>" + d_FWHM["FWIA"] +" && EIB>" + d_FWHM["FWIB"] +"&& EIC<" + d_FWHM["FWIC"] +"&& EID<" + d_FWHM["FWID"] + "&& abs(EC1-EC2)<2 &&" +  d_estimator["Q_S1"] + "<0.75 && " + d_estimator["Q_S1"] + ">0.2&& (EIB>1 || EID>1)")
    ...
    pop_file_name = bolo_name + "_BetaS1_ion_cut.txt"
    pop_file = script_utils.open_text_file(pop_path_name, pop_file_name, 'w')
    for k in range(pop_len):
    ...
    tree.Draw(">>l_BetaS2",standard_cuts +  "&&" + d_estimator["HEAT"] + "<40 &&" + d_estimator["HEAT"] + ">0 && EIA<" + d_FWHM["FWIA"] +" && EIB<" + d_FWHM["FWIB"] +"&& EIC>" + d_FWHM["FWIC"] +"&& EID>" + d_FWHM["FWID"] + "&& abs(EC1-EC2)<2 &&" +  d_estimator["Q_S2"] + "<0.75 && " + d_estimator["Q_S2"] + ">0.2&& (EIB>1 || EID>1)")
    pop_len = l_BetaS2.GetN()
    pop_file_name = bolo_name + "_BetaS2_ion_cut.txt"
    pop_file = script_utils.open_text_file(pop_path_name, pop_file_name, 'w')
    for k in range(pop_len):
    ...
    # S 1     E V E N T S
    tree.Draw(">>l_PbS1",standard_cuts +  "&&" + d_estimator["HEAT"] + "<40 &&" + d_estimator["HEAT"] + ">0 &&  EIA>" + d_FWHM["FWIA"] +" && EIB>" + d_FWHM["FWIB"] +"&& EIC<" + d_FWHM["FWIC"] +"&& EID<" + d_FWHM["FWID"] + "&& abs(EC1-EC2)<2 &&" +  d_estimator["Q_S1"] + "<0.2 && (EIB>1 || EID>1) &&"  + d_estimator["Q_S1"] + ">0.04" )
    print 
    pop_len = l_PbS1.GetN()
    pop_file_name = bolo_name + "_PbS1_ion_cut.txt"
    pop_file = script_utils.open_text_file(pop_path_name, pop_file_name, 'w')
    for k in range(pop_len):
    ...
    # S 2     E V E N T S
    tree.Draw(">>l_PbS2",standard_cuts +  "&&" + d_estimator["HEAT"] + "<40 &&" + d_estimator["HEAT"] + ">0 && EIA<" + d_FWHM["FWIA"] +" && EIB<" + d_FWHM["FWIB"] +"&& EIC>" + d_FWHM["FWIC"] +"&& EID>" + d_FWHM["FWID"] + "&& abs(EC1-EC2)<2 &&" +  d_estimator["Q_S2"] + "<0.2 && (EIB>1 || EID>1) &&"  + d_estimator["Q_S2"] + ">.04" )
    pop_len = l_PbS2.GetN()
    pop_file_name = bolo_name + "_PbS2_ion_cut.txt"
    pop_file = script_utils.open_text_file(pop_path_name, pop_file_name, 'w')
    for k in range(pop_len):
