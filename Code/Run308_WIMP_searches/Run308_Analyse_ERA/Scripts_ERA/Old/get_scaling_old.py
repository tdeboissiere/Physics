...
def get_scaling(bolo_name, duration, heat_inf):
	...
	exp_heat = 2*tree.GetEntries(standard_cuts + "&& 0.5*(EIB+EID)<0 && EC>" + str(ECinf))
	tree.Draw("0.5*(EIB+EID):0.5*(EC1+EC2)>>hist(100,0.5,15,100,-1,15)", standard_cuts)
	print exp_heat, tree.GetEntries(standard_cuts +"&& EIA<1.4 && EIC<1.4 && 0.5*(EIB+EID)<0.7&& EC>" + str(ECinf))
	raw_input()

	fFidGamma.Integral(4,15)
	print fFidGamma.Integral(4,15), tree.GetEntries(standard_cuts +"&& EIA<1.4 && EIC<1.4 && 0.5*(EIB+EID)>1 && EC>4 && abs(EIB-EID)<1")
...
def get_scaling_with_neutron(bolo_name, duration, heat_inf):
	...
	exp_heat = 2*tree.GetEntries(standard_cuts + "&& 0.5*(EIB+EID)<0 && 0.5*(EC1+EC2)>" + str(ECinf))
	...
	with open(file_path_name + bolo_name + "_bckg_scaling_with_neutron_no_ion_cut_" + str(ECinf) + "_heat_thresh.txt", "w") as fs:
		fs.write("#no ion cut, 0.5*(EC1+EC2)>0, rate in days, " + str(ECinf)  + " to 15 keV in heat\n")
