...
def skim_true_event_tree_with_heatrate(bolo_name, d_cut, analysis_type, data_dir, tree_name = "data"):
	...
	veto_cut = "EIA<" + str(d_cut["sigma_vet"]) + "*" + d_std["FWIA"] + "&&" + "EIC<" + str(d_cut["sigma_vet"]) + "*" + d_std["FWIC"]
	...
	file_heatrate = TFile("../Analyse_" + bolo_name + "/ROOT_files/" + bolo_name +  "_heatonly_rate_over_time.root", "read") 
	heatrate      = file_heatrate.Get("hist_heatrate")

	file_skim = TFile(data_dir+bolo_name + "_" + analysis_type  +"_with_heat_rate_fond.root", "recreate")
	skim_tree = TTree("data", "data")
...
def skim_true_event_tree(bolo_name, d_cut, analysis_type, data_dir, tree_name = "data"):
	...
	file_tree     = TFile(data_dir+bolo_name+"_PSA_and_fond.root")
	tree          = file_tree.Get(tree_name)
	...
	ion_cut  = str(d_cut["EIinf"]) + "<" + d_est["FID"] + "&&" + str(d_cut["EIsup"]) + ">" + d_est["FID"]
	veto_cut = "EIA<(1./2.7)*" + str(d_cut["sigma_vet"]) + "*" + d_std["FWIA"] + "&&" + "EIC<(1./2.7)*" + str(d_cut["sigma_vet"]) + "*" + d_std["FWIC"]
	PSA_cut = "pearsA>0.91 && dtw_valA<100"

	all_cuts = "&&".join([standard_cuts, heat_cut, ion_cut, veto_cut, PSA_cut])
	print all_cuts.split("&&")
	...
	pop_len       = l_event.GetN()
 	...
 	file_skim = TFile(data_dir+bolo_name + "_" + analysis_type  +"_PSA_and_fond.root", "recreate")
	skim_tree          = TTree("data", "data")
...
skim_true_event_tree(bolo_name, d_cut, analysis_type, data_dir)
