...
# TODOs...
# - menu deroulant pour choix fichier
# - debug "find template" (offset en temps sur l'ion)
# .. et pour wiener fit, faire plot A/chi2(offset)
# - aligner les items : truc.grid(row=0,column=1)
...
class EventDisplay:
    def __init__(self,tkroot,anadir,bolo):
        ...
        self.checkbutton_stacktmplt.pack(side=Tk.TOP)
...
#    gSubPlot.autoscale(tight=True)
    ...
    def wienerfit(self):
        ...
        noise=RunParams.GetNoiseSpectrum(noisefilename,self.Channel,self.Evt.DateSec())
    ...
    def fit_template(self):
        if self.Channel.find("chal") != -1 :
            params=self.fit_template_chal()
            self.tmplt_type="StandardChal"
        else :
            params=self.fit_template_ion()
            self.tmplt_type="StandardIon"
            if self.Evt.Pulse(self.Channel).IsBBv2()==1 : self.tmplt_type="IonBB2"
        if self.TkVarStackTmplt.get()==0 :
            ...
            if write_yes=="y" :
                ...
                for par in params[:len(params)-2] : f.write(" {0:.3f}".format(par))
                f.write("\n")
                f.close()
        else :
            ...
            if write_yes=="y" :
                if self.Channel!=self.VoieListCoefTmplt:
                    ...
                    self.ListCoefTmplt=[]
                self.ListCoefTmplt.append(params[:len(params)-2])

    def fit_template_from_trace(self,trace,offset_est,ampl_est):
        nbpts=np.arange(len(trace))
        x=np.arange(nbpts)
        y=np.asarray(trace)
        if self.tmplt_type=="StandardChal" :
            ...
            par0=np.array([c0_est,c1_est,c2_est,c3_est,c4_est,c5_est,offset_est,ampl_est])
            ...
            par0=np.array([c0_est,c1_est,c2_est,offset_est,ampl_est])
            ...
            par0=np.array([c0_est,offset_est,ampl_est])
    ...
    def fit_template_ion(self):
        p=self.Evt.Pulse(self.Channel)
        pp=FitPulse(p)
        pp.BasicPreprocess(0)
        nbpts=pp.TraceLength()
        x=np.arange(nbpts)
        y=np.asarray(pp.ProcessedTrace())
        offset_est=float(pp.SimpleAmplBin())
        if pp.IsBBv2()==0 :
            c0_est=1500
            c1_est=0.15
            c2_est=0.01
            ampl_est=pp.SimpleAmpl()/(1-c1_est)
            #print "Initial guess:",c0_est,c1_est,c2_est,offset_est,ampl_est
            par0=np.array([c0_est,c1_est,c2_est,offset_est,ampl_est])
            popt,pcov=curve_fit(TemplateIon,x,y,p0=np.array(par0))
            print "Fit results: {0:.3f} {1:.3f} {2:.3f} {3:.3f} {4:.3f}".format(popt[0],popt[1],popt[2],popt[3],popt[4])
            ...
            c0_est=0.01
            ampl_est=pp.SimpleAmpl()
            # print "Initial guess:",c0_est,offset_est,ampl_est
            par0=np.array([c0_est,offset_est,ampl_est])
            popt,pcov=curve_fit(TemplateIonBB2,x,y,p0=np.array(par0))
            print "Fit results: {0:.3f} {1:.3f} {2:.3f}".format(popt[0],popt[1],popt[2])
            z=TemplateIonBB2(x,popt[0],popt[1],popt[2])
        self.SubPlot.plot(x,z,color="red")
        self.SubPlot.plot(np.arange(nbpts),pp.ProcessedTrace(),color="blue")
        ymin=min(pp.ProcessedTrace())-5
        ymax=max(pp.ProcessedTrace())+5
        xlen=pp.TraceLength()
        self.SubPlot.axis([0,xlen,ymin,ymax])
        #    gSubPlot.autoscale(tight=True)
        self.Canvas.show()
        return popt

    def fit_template_chal(self):
        p=self.Evt.Pulse(self.Channel)
        pp=FitPulse(p)
        pp.BasicPreprocess(0)
        offset_est=float(pp.SimpleAmplBin()-10)
        c0_est=5.
        c1_est=25.
        c2_est=0.2
        c3_est=150.
        ...
        xlen=pp.TraceLength()
        self.SubPlot.axis([0,xlen,ymin,ymax])
        #    self.SubPlot.autoscale(tight=True)
        self.Canvas.show()
        return popt
    ...
    def plot_template_coefs(self):
        ...
        for coefs in self.ListCoefTmplt :
            ...
            self.SubPlot.plot(xx,z,color='blue')
        meanz=[t/len(self.ListCoefTmplt) for t in meanz]
        ## ESSAI = FIT DE MEANZ (dont la forme, avant fit, est satisfaisante)
        

        ## ESSAI AVEC COEF MEDIAN : MOYENNEMENT SATISFAISANT
        ...
        #        print coef_median
        #        if self.tmplt_type=="StandardChal":
        #            zmedian=TemplateChal(xx,coef_median[0],coef_median[1],coef_median[2],coef_median[3],coef_median[4],coef_median[5],offset,1)
        #        elif self.tmplt_type=="StandardIon":
        #            zmedian=TemplateIon(xx,coef_median[0],coef_median[1],coef_median[2],offset,1)
        #        elif self.tmplt_type=="IonBB2":
        #            zmedian=TemplateIonBB2(xx,coef_median[0],offset,1)
        #        norm=max(zmedian)
        #        zmedian=[t/norm for t in zmedian]
        #        self.SubPlot.plot(xx,zmedian,color='green')
        self.SubPlot.plot(xx,meanz,color='red')
        self.SubPlot.axis([0,self.Pulse.TraceLength(),-1.2,1.2])
        self.Canvas.show()
        #    write_yes=raw_input("Write template to file? (y/n)")
        write_yes="n"
        if write_yes=="y" :
            ...
            f=open(file,"a")
            f.write('"'+self.Channel+'" '+self.tmplt_type+" 0 0")
            for par in coef_median[:len(coef_median)-2] : f.write(" {0:.3f}".format(par))
            f.write("\n")
            f.close()
