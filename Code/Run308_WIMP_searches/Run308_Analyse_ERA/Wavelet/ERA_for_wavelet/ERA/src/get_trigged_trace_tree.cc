...
// #include "TSpline5.h"
#include <algorithm> 
#include "/sps/edelweis/tmain/Wavelet_computations/ERA_for_wavelet/ERA/src/EdwEvent.h"
#include "/sps/edelweis/tmain/Wavelet_computations/ERA_for_wavelet/ERA/src/EdwPulse.h"
#include "/sps/edelweis/tmain/Wavelet_computations/ERA_for_wavelet/ERA/src/EdwTemplate.h"
#include "/sps/edelweis/tmain/Wavelet_computations/ERA_for_wavelet/ERA/src/EdwUtils.h"
#include "/sps/edelweis/tmain/Wavelet_computations/ERA_for_wavelet/ERA/src/NoiseSpectrum.h"
#include "/sps/edelweis/tmain/Wavelet_computations/ERA_for_wavelet/ERA/src/FitPulse.h"
...
int get_trigged_trace_tree(TString bolo_name, string run_id)
{
	
  // gSystem->Load("/home/irfulx204/mnt/tmain/Desktop/New_ERA/ERA/lib/EraLib.so");
  TString gen_dir("/sps/edelweis/tmain/Wavelet_computations/");
  ...
  // Code for test of trigger
  // for (int i = 0; i < nEntries; ++i)
  // {
  //   Tree->GetEntry(i);
    // cout <<i << "  " << Evt->NbPulses() << endl;

    // if (Evt->NbPulses()==6) {cout << i << "  " << Evt->Pulse(0)->IsTrig() << " " << Evt->Pulse(1)->IsTrig() << endl;}

    // for (UInt_t k = 0; k<Evt->NbPulses(); k++)
    // {
    //   if (Evt->Pulse(k) != NULL)
    //   {
    //     cout << " " << k << " " << Evt->Pulse(k)->Channel() << " " ;
    //   }
    // }
    // cout << endl;
  // }
  ...
  TString outdir =gen_dir + bolo_name + TString("/") + bolo_name +  TString("_") + Run_ID + TString("_trigged_trace_tree.root");
  ...
  for (int i = 0; i < nEntries; ++i)
  {
    ...
    if (Evt->NbPulses() == 6) 
    {
      ...
      {
        if (pulseA->IsTrig()==1 || pulseB->IsTrig()==1) 
        {
