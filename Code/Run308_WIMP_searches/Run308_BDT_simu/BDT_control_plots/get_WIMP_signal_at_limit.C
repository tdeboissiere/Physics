...
TF1* f1 = new TF1("S0",computerate_sfg,0,30.7,4);	
...
int main() {
	...
	vlim[2] = (5E-4);
	vlim[3] = (9E-5);
	vlim[4] = (2.03112465854e-05);
	vlim[5] = (1.65138099679e-06);
	vlim[6] = (4.91755113293e-07);


	TString analysis = TString("ana_0.5_min4_5");

	ofstream file_num(TString("./Text_files/WIMP_counts_for_35kgday_") + analysis + TString("_2D.txt"));
	...
	for(std::vector<int>::iterator it = vmass.begin(); it != vmass.end(); it++){
		...
		//15 keVee = 30.7 keVNR
		double xmin=1.22 ;double xmax=30.7; double ymin=-4 ; double ymax=15.0;
		int binX=200; int binY=200;
		float binXf=200.; float binYf=200.;
		TString  s_Wmass= Form("%d", *it);
		//Increase the bounds of TF2 to avoid numerical problems
		TF2* PDF = new TF2(TString("WIMP_") + s_Wmass + TString("_GeV"), PDF2D,  xmin, 32, ymin, 16, 2);
		// cout << PDF << endl;
		// cout << PDF->Eval(5,5) << endl;
		PDF->SetParameter(0,*it);
		PDF->SetParameter(1,vlim[counter]);
		PDF->SetNpx(binX);
		PDF->SetNpy(binY);  
