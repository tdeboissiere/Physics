...
def build_BetaPb_tree(bolo_name, event_type, num_event):
    ...
    if event_type == "S1Beta":
        for i in range(num_event):
            ...
            evt_heat = func_S1Beta.GetRandom(1,30)
            strag = func_stragS1Beta.Eval(evt_heat)
            strag=evt_heat*0.1

            EIA[0]=func_convS1Beta.Eval(evt_heat) + np.random.normal(0, TMath.Sqrt((d_std_true_events["FWIA"]+strag)**2 - d_std_true_events["FWC1_ERA"]**2))
            EIC[0]=np.random.normal(0, TMath.Sqrt((d_std_true_events["FWIC"])**2 - d_std_true_events["FWC1_ERA"]**2))

            EIB[0]=func_convS1Beta.Eval(evt_heat) + np.random.normal(0, TMath.Sqrt((d_std_true_events["FWIB"]+strag)**2 - d_std_true_events["FWC1_ERA"]**2))
            EID[0]=np.random.normal(0, TMath.Sqrt((d_std_true_events["FWID"])**2 - d_std_true_events["FWC1_ERA"]**2))

            EC1[0]=evt_heat #+  np.random.normal(0, 5) #+ np.random.normal(0, d_std_true_events["FWC1_ERA"])
