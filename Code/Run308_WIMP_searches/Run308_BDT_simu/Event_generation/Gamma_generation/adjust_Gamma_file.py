...
def adjust_Gamma_file(bolo_name, d_cut, analysis_type, d_event_type_num_event):
    ...
    with open(gen_path + "Event_generation/Gamma_generation/build_Gamma_tree.C", "r") as f_cpp:
        list_cpp_lines = f_cpp.readlines()
   
    with open(gen_path + "Event_generation/Gamma_generation/build_Gamma_tree.C", "w") as f_cpp:
        for line in list_cpp_lines:
            ...
            #     f_cpp.write("\t\t\tfloat EI =" + str(d_estimator["FID"][:5]) + "*EIB+" +str(1-float(d_estimator["FID"][:5]) )+ "*EID;\n")
                ...
                f_cpp.write("\tnum_event[0] =" + str(d_event_type_num_event["FidGamma"]) + ";\n")
            elif "num_event[1] =" in line:
                f_cpp.write("\tnum_event[1] =" + str(d_event_type_num_event["S1Gamma"]) + ";\n")
            elif "num_event[2] =" in line:
                f_cpp.write("\tnum_event[2] =" + str(d_event_type_num_event["S2Gamma"]) + ";\n")
            else:
                f_cpp.write(line)
