...
double sigma_ion= 0.9/2.3548;  //ion res in keV
double sigma_rec= 0.5;   // heat res in keVNR
...
TF1* f1 = new TF1("S0",computerate_sfg,0,29,3);	
...
//Do the integral with a histogram
//TH1F* hRate= new TH1F("hRate","hRate",2000, 0, 29);
// for (int i = 1; i <=2000; i += 1) {
//     	hRate->SetBinContent(i,f1->Eval(29*i/2000.));	
//     }
// double Rate = (15.0/2000.)*hRate->Integral();
//delete hRate;


double eff_ER = 0.5*(1+TMath::Erf((Er-0.5)/(0.35*1.4142)));
if (Ei>1.5)	{return Rate*eff_ER;}
else {return 0;}
...
int main() {
	...
	// TFile filou("/home/irfulx204/mnt/tmain/Desktop/Run308_BDT/BDT_FID837/ion_cut_1keV/ROOT_files/FID837_ion_eff.root", "read");
	// f_ion_eff = (TF1*)filou.Get("ion_eff");

	// double exposure = 42.7;

	//Mass vector and cut efficiency
	vector<double> vmass;
	...
	ofstream file_num("./Text_files/WIMP_counts_for_1kgday_fid_cut_1_5keV.txt");


	for(std::vector<double>::iterator it = vmass.begin(); it != vmass.end(); it++){

		//1 keVee = 2.44 keVNR
		double xmin=2.44 ;double xmax=29.0; double ymin=0 ; double ymax=15.0;
		int binX=200; int binY=100;
		float binXf=200.; float binYf=100.;
		TF2* PDF = new TF2("PDF2D", PDF2D,  xmin, xmax, ymin, ymax, 1);
