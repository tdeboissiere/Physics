...
int main() {
   ...
   //General path
   TString gen_path = TString("/home/irfulx204/mnt/tmain/Desktop/Run308_BDT_simu_corr/");
   TString Ana_path = TString("/home/irfulx204/mnt/tmain/Desktop/Run308_Neutron/Neutron_ERA_merged/");
   ...
   for(std::vector<int>::iterator it = Wmass_vec.begin(); it != Wmass_vec.end(); it++) {
         ...
         TString sfile_hist = app_path + TString("/real_neutron_mass_") + s_Wmass +TString("_hist.root");
         TString sfile_tree = app_path + TString("/real_neutron_mass_") + s_Wmass +TString("_tree.root");
         remove(sfile_hist);
         remove(sfile_tree);
   }
   ...
   for(std::vector<int>::iterator it = Wmass_vec.begin(); it != Wmass_vec.end(); it++) {
         ...
         TString  s_Wmass= Form("%d", *it);
         //Input files
         TString sData          = Ana_path + bolo_name + TString("_") + analysis_type + TString("_weighted_mass_") + s_Wmass + TString("_neutron.root");
         //For the hist output
         TString sData_out      = app_path + TString("/real_neutron_mass_") + s_Wmass +TString("_hist.root");
         //For the tree output
         TString sData_out_tree = app_path + TString("/real_neutron_mass_") + s_Wmass +TString("_tree.root");
