...
void TMVAClassificationApplication(int Wmass, TString fname, TString fout, TString fout_tree, TString tree_name, TString bolo_name, TString simu_index, TString analysis_type) 
{   
#ifdef __CINT__
...
int main() {
   ...
   for(std::vector<int>::iterator it = Wmass_vec.begin(); it != Wmass_vec.end(); it++) {
         ...
         TString sfile_hist = app_path + TString("/data_nosurf_mass_") + s_Wmass +TString("_hist.root");
         TString sfile_tree = app_path + TString("/data_nosurf_mass_") + s_Wmass +TString("_tree.root");
         remove(sfile_hist);
         remove(sfile_tree);
   }
   ...
   for(std::vector<int>::iterator it = Wmass_vec.begin(); it != Wmass_vec.end(); it++) {
      for (int k = 0; k < nsimu ; ++k)
      {
         ...
         //Input files
         TString sData          = out_path + TString("/True_events/ROOT_files/") + bolo_name + TString("_true_events_nosurf_tree.root");
         //For the hist output
         TString sData_out      = app_path + TString("/data_nosurf_mass_") + s_Wmass +TString("_hist.root");
         //For the tree output
         TString sData_out_tree = app_path + TString("/data_nosurf_mass_") + s_Wmass +TString("_tree.root");

         TMVAClassificationApplication(*it, sData,sData_out,sData_out_tree, TString("t_new") + s_index, bolo_name, s_index, analysis_type);
      }
