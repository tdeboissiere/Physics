...
def adjust_MVA_files(bolo_name, analysis_type, nsimu):
    ...
    with open(gen_path + "MVA/TMVAClassification.C", "r") as f_cpp:
        list_cpp_lines = f_cpp.readlines()
   
    with open(gen_path + "MVA/TMVAClassification.C", "w") as f_cpp:
    ...
    with open(gen_path + "MVA/TMVAindividual.C", "r") as f_cppi:
        list_cpp_lines = f_cppi.readlines()
   
    with open(gen_path + "MVA/TMVAindividual.C", "w") as f_cppi:
        for line in list_cpp_lines:
    ...
    with open(gen_path + "MVA/TMVAtruedata.C", "r") as f_cppd:
        list_cpp_lines = f_cppd.readlines()
   
    with open(gen_path + "MVA/TMVAtruedata.C", "w") as f_cppd:
        for line in list_cpp_lines:
            ...
            if "TString bolo_name = TString" in line:
                ...
                f_cppd.write('\tint nsimu =' + str(nsimu) + ';\n')                 
            else:
                f_cppd.write(line)

    with open(gen_path + "MVA/TMVArealtruedata.C", "r") as f_cppd:
        list_cpp_lines = f_cppd.readlines()

    with open(gen_path + "MVA/TMVArealtruedata.C", "w") as f_cppd:
        for line in list_cpp_lines:
